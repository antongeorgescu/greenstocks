...
Greenstocks Investment Advisor (GIA) is built as a simple,user friendly web application that allows the user, through only a few clicks, to discover and invest in stocks that comply to ESG policies. Down below you can see a normal use of the prototype that we built.<br/>
Note:&nbsp;Since this is only a prototype, we have left aside a bunch of features that are important and can be further developed, like:
...
Business Engine contains the MVC model functionality. It includes a powerful Natural Language Processing (NLP) library based on Python NLTK (Natural Language Toolkit). <br/>
